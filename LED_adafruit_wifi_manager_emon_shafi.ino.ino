...
//#define WLAN_SSID       "Limited Internet"
//#define WLAN_PASS       "pythonkali123"

/************************* Adafruit.io Setup *********************************/

#define AIO_SERVER      "io.adafruit.com"
#define AIO_SERVERPORT  1883                   // use 8883 for SSL
#define AIO_USERNAME    "Nabilphysics"
#define AIO_KEY         "b11f72ad18c44d83b536a0707e6fe394"
...
// Notice MQTT paths for AIO follow the form: <username>/feeds/<feedname>
Adafruit_MQTT_Publish onoffl1state = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/onoffl1state");
Adafruit_MQTT_Publish onoffl2state = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/onoffl2state");
Adafruit_MQTT_Publish onoffl3state = Adafruit_MQTT_Publish(&mqtt, AIO_USERNAME "/feeds/onoffl3state");
...
void loop() {
 ...
 // publishing first light'state
  lightOneState = digitalRead(LED1);
  Serial.print(F("\nSending lightOne val "));
  Serial.print(lightOneState);
  Serial.print("...");
  if (! onoffl1state.publish(lightOneState)) {
  ...
  //publishing second light's state
   lightTwoState = digitalRead(LED2);
  Serial.print(F("\nSending lightTwo val "));
  Serial.print(lightOneState);
  Serial.print("...");
  if (! onoffl2state.publish(lightTwoState)) {
    Serial.println(F("Failed"));
  } else {
    Serial.println(F("OK!"));
  }

  // publishing third light's state
    lightThreeState = digitalRead(LED3);
  Serial.print(F("\nSending lightThree val "));
  Serial.print(lightThreeState);
  Serial.print("...");
  if (! onoffl3state.publish(lightThreeState)) {
    Serial.println(F("Failed"));
  } else {
    Serial.println(F("OK!"));
  }
